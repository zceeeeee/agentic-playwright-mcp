...
**推荐方式**：使用桌面客户端（Electron），安装后直接运行 `browser-agent-desktop.exe`。

**备用方式**：Web GUI（`browser-agent gui --port 8081`），适合无桌面环境的服务器。
...
安装后运行 `browser-agent-desktop.exe`，提供完整的可视化操作界面，支持浏览器引擎选择、任务管理、设置等。
...
适合无桌面环境的服务器，功能与桌面客户端一致，但不再作为主要维护对象。
...
启动客户端时如果没有配置 API Key，会自动弹出引导界面。
